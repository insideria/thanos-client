...
class ReadPreLoadConfig : Singleton<ReadPreLoadConfig>
{
     ...
     public void PreLoad()
    {
        ...
        {                       
            GameObject preObj = ObjectPool.Instance.GetGO(file);
            try
            {
                ObjectPool.Instance.ReleaseGO(file, preObj, PoolObjectTypeEnum.Effect);
            } catch(Exception e) {
                Debugger.LogError("PreLoad ReleaseGO Error file: " + file);                
            }
        }
        ...
        {
            GameObject tipObj = ObjectPool.Instance.GetGO(tipName);
            tipObj.name = tipName;
        ...
        {
            ObjectPool.Instance.ReleaseGO(tipObj.name, tipObj, PoolObjectTypeEnum.UITip);
        }         
    }
