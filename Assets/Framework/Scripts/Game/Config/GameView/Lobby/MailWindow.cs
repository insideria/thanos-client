...
namespace Thanos.View
{
    public class MailWindow : BaseWindow
    {
        ...
        {
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_MailEnter, Show);
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_MailExit, Hide);
        ...
        {
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_MailEnter, Show);
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_MailExit, Hide);
        ...
        {
            EventCenter.AddListener<Mail>((Int32)GameEventEnum.GameEvent_UpdateMailInfo, UpdateMail); 
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_UpdateMailList, ResetMail);
        }

        //游戏事件注消
        protected override void OnRemoveListener()
        {
            EventCenter.RemoveListener<Mail>((Int32)GameEventEnum.GameEvent_UpdateMailInfo, UpdateMail);
            
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_UpdateMailList, ResetMail);
        }
        ...
        {
            List<KeyValuePair<int, Mail>> mailList = MailModel.Instance.MailList;
            if (mailList != null && mailList.Count > 0)
            {
                foreach (var kv in mailList)
                {
                    ...
                    ResourceItem objUnit = ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadMailTitleGUI, ResourceType.PREFAB);
                    ...
                    int start = kv.Value.mTitle.IndexOf("]") + 1;
                    int end = kv.Value.mTitle.LastIndexOf("[");
                    string mailText = kv.Value.mTitle.Substring(start, end - start);
                    ...
                    if (mailText.Length >= 6)
                    {
                        ...
                        newMailTitle = kv.Value.mTitle.Replace(mailText, newMailTitle);
                    }
                    else
                    {
                        newMailTitle = kv.Value.mTitle;  
                    }

                    //如果查看过的 则设置颜色为灰色 
                     mail.readedMailName.text = mail.currMailName.text = newMailTitle;
                     if (kv.Value.CurrentState != MailCurtStateEnum.New)
                     {
                    ...
                    if ( (LastObj == null)  || (LastObj.currMailId <= mail.currMailId && kv.Value.CurrentState == MailCurtStateEnum.New) )
                    {
                        LastObj = mail;
                        curtActiveObj = obj;
                    }
        ...
        public void UpdateMail(Mail mdb)
        {
            if (GoldList.Count != 0)
            {
            ...
            if ((curMailObj != null) && (mdb != null) && (curtMailId == mdb.mId))
            {
                curMailObj.mMailSenderTxt.text = mdb.mSender;
                curMailObj.mMailTitleTxt.text = mdb.mTitle;
                curMailObj.mMailContentTxt.text = mdb.mContent;
                curMailObj.mMailCreateTimeTxt.text = mdb.mCreateTime;
                mTemp.gameObject.SetActive(true);
                string[] giftArray = mdb.mGift.Split('|');
                for (int i = 0; i < giftArray.Length; i++)
                {
                    ...
                    if (giftStr.Length == 3)
                    {
                        ...
                        MailGiftTypeEnum giftType = (MailGiftTypeEnum)Convert.ToInt32(giftStr[0]);
                        switch (giftType)
                        {
                            case MailGiftTypeEnum.Gold:
                                {
                                    obj.GetComponent<UISprite>().spriteName = "9";
                                    obj.transform.Find("Label").GetComponent<UILabel>().text = giftStr[2];
                                } break;
                            case MailGiftTypeEnum.Diamond:
                                {
