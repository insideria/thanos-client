...
namespace Thanos.View
{
    public class ChatTaskWindow : BaseWindow
    {
        ...
        {
            EventCenter.AddListener<UInt64>((Int32)GameEventEnum.GameEvent_ChatTaskEnter, ShowFriendTask);
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_ChatTaskExit, Hide);
        ...
        {
            EventCenter.RemoveListener<UInt64>((Int32)GameEventEnum.GameEvent_ChatTaskEnter, ShowFriendTask);
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_ChatTaskExit, Hide);
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_LobbyExit, RemoveTask);
        }
            ...
            else
            {
                foreach (var item in FriendManager.Instance.AllTalkDic.Values)
                {
                    ...
                    if (item.NickName == null && string.IsNullOrEmpty(item.NickName))
                        continue;
                    if (TalkChatName.TryGetValue(item.GUID, out gameObj))
                    {
        ...
        void ClearOld()
        {
            ...
            {
                cat.SetTalkState(GameDefine.MsgTalkEnum.ReadMsg);
                foreach (var item in cat.ChatInfoList)
                {
                    ShowTalk(mLastTemp,item.NickName, item.mMsg, item.mHead, item.mIsLocalPlayer);
                }
            }
        }
        ...
        void ViableTalk(string currStr)
        {
            ...
            if (FriendManager.Instance.AllFriends.TryGetValue(mLastTemp, out friend))
            {
                if (!FriendManager.Instance.AllTalkDic.TryGetValue(mLastTemp, out cat))
                {
                    ...
                    cat.SetMsgInfo(mLastTemp, friend.NiceName, currStr, MsgTalkEnum.ReadMsg, GameUserModel.Instance.GameUserHead, true);
        ...
        {
            ResourceItem objUnit = ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadTalk, ResourceType.PREFAB);
            GameObject obj = GameObject.Instantiate(objUnit.Asset) as GameObject;
        ...
        public void ShowTalkInfo(UInt64 sGUID)
        {
            ...
            if (mLastTemp == sGUID)
            {
                ...
                {
                    cat.SetTalkState(GameDefine.MsgTalkEnum.ReadMsg);
                    ShowTalk(sGUID,cat.NickName, cat.ChatInfoList[cat.ChatInfoList.Count - 1].mMsg, cat.ChatInfoList[cat.ChatInfoList.Count - 1].mHead, cat.ChatInfoList[cat.ChatInfoList.Count - 1].mIsLocalPlayer);
                }
                ChatTaskCtrl.Instance.SetNewChat();
            }
            else
        ...
        void Refresh()
        {
            ...
            foreach (var item in FriendManager.Instance.AllTalkDic.Values)
            {
                if (TalkChatName.TryGetValue(item.GUID, out obje))
                {
                    if (item.TalkState == MsgTalkEnum.UnReadMsg)
                    {
        ...
        {
            EventCenter.AddListener<UInt64>((Int32)GameEventEnum.GameEvent_ReadyChatTaskEnter, CreateMember);
            EventCenter.AddListener<UInt64>((Int32)GameEventEnum.GameEvent_ReceiveNewMsg, ShowTalkInfo);
            EventCenter.AddListener<UInt64>((Int32)GameEventEnum.GameEvent_RemoveChatTask, RemovChat);
        }
        ...
        {
            EventCenter.RemoveListener<UInt64>((Int32)GameEventEnum.GameEvent_ReadyChatTaskEnter, CreateMember);
            EventCenter.RemoveListener<UInt64>((Int32)GameEventEnum.GameEvent_ReceiveNewMsg, ShowTalkInfo);
            EventCenter.RemoveListener<UInt64>((Int32)GameEventEnum.GameEvent_RemoveChatTask, RemovChat);
        }
