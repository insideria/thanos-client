...
namespace Thanos.View
{
    public class RuneRefreshWindow : BaseWindow
    {
        ...
        {
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_RuneRefeshWindowEnter, Show);
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_RuneRefeshWindowExit, Hide);
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_HomePageExit, ParentClose);
        }
        ...
        {
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_RuneRefeshWindowEnter, Show);
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_RuneRefeshWindowExit, Hide);
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_HomePageExit, ParentClose);
        }
        ...
        private void LoadOneRefreshCard(uint refreshID, int num)
        {
            ...
            ResourceItem objUnit = ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadRuneRefreshCardRes, ResourceType.PREFAB);
        ...
        public void onClickWash()
        {
            ...
            if (m_RefreshCardSlotInfo != null)
            {
                HolyGameLogic.Instance.AskRefresh(m_RuneSlotInfo.runeID, m_RefreshCardSlotInfo.runeID);
                return;
            }
            ...
            EventCenter.Broadcast<EMessageType, string, Action<bool>>((Int32)GameEventEnum.GameEvent_ShowLogicMessage, EMessageType.EMT_SureToWash, showStr, windowsCallBack);
        }
        ...
        public void windowsCallBack(bool flag)
        {
            if (flag)
            {
                ...
                HolyGameLogic.Instance.AskRefresh(m_RuneSlotInfo.runeID, 0);
            }
        }
        ...
        {
            EventCenter.AddListener<uint, int, long>((Int32)GameEventEnum.GameEvent_RuneBagUpdate, RefeshRuneBagInfo);
            EventCenter.AddListener<uint, int, long>((Int32)GameEventEnum.GameEvent_RuneRefreshCardUpdate, RefeshCardInfo);
        ...
        {
            EventCenter.RemoveListener<uint, int, long>((Int32)GameEventEnum.GameEvent_RuneBagUpdate, RefeshRuneBagInfo);
            EventCenter.RemoveListener<uint, int, long>((Int32)GameEventEnum.GameEvent_RuneRefreshCardUpdate, RefeshCardInfo);
        ...
        public override void Update(float deltaTime)
        {
            if (m_SuccessTime > 0)
            {
                if (Time.time - m_SuccessTime >= 0.8)
                {
                    ...
                    EventCenter.Broadcast<EPurchaseType, string, string, int>((Int32)GameEventEnum.GameEvent_PurchaseRuneSuccessWin, EPurchaseType.EPT_Rune, runeCfg.RuneName, runeCfg.Icon, 1);
                    m_SuccessTime = -1;
                }
            }
        }
