...
namespace Thanos.View
{
    public class PresonInfoWindow : BaseWindow
    {
        ...
        {
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_PresonInfoEnter, Show);
        ...
        {
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_PresonInfoEnter, Show);
        ...
        private void ShowChangeHeadIcon(GameObject go)
        {
            ...
            foreach (var item in heroInfoList)
            {
                foreach (var mTemp in item.HeroHeadType)
                {
                    ...
                    ResourceItem objUnit = ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadHeadPhotoUi, ResourceType.PREFAB);
        ...
        {
            if (Thanos.Ctrl.PresonInfoCtrl.Instance.GetTimeExp() && PresonInfoCtrl.Instance.GetExpLastMin())
                ChangeExpTime();
            if (Thanos.Ctrl.PresonInfoCtrl.Instance.GetTimeGold() && PresonInfoCtrl.Instance.GetGoldLastMin())
                ChangeGoldTime();
        }
        ...
        {
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_ChangeNickName, ChangeNickName);
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_ChangeHeadID, ChangeHeadID);
        }
        ...
        public override void OnEnable()
        {
            ...
            if (Thanos.Ctrl.PresonInfoCtrl.Instance.GetTimeExp())
                ChangeExpTime();
            if (Thanos.Ctrl.PresonInfoCtrl.Instance.GetTimeGold())
                ChangeGoldTime();
