...
namespace Thanos.View
{
    public class ScoreWindow : BaseWindow
    {
        ...
        {
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_ScoreEnter, Show);
            EventCenter.AddListener((Int32)GameEventEnum.GameEvent_ScoreExit, Hide);
        }

        public override void Realse()
        {
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_ScoreEnter, Show);
            EventCenter.RemoveListener((Int32)GameEventEnum.GameEvent_ScoreExit, Hide);
        }
        ...
        public void  RefreshData()
        {
            ...
            foreach(var item in ScoreCtrl.Instance.GetSettlementList())
            {
                if (item.mCamp == EntityCampTypeEnum.A)
                {
                ...
                else if (item.mCamp == EntityCampTypeEnum.B)
                {
                    if (num <= 5)
        ...
        {
            //HolyTech.AudioManager.Instance.StopBgAudio();
            IPlayer player = PlayerManager.Instance.LocalPlayer;
            if (player == null)
                player = PlayerManager.Instance.LocalAccount;
            if (player.StateSituation == SITUATION.WIN)
            {
                //HolyTech.AudioManager.Instance.PlayLongVoiceAudio(Resources.Load(AudioDefine.PATH_HERO_KILLSOUND + "Victory") as AudioClip);

                ResourceItem clipUnit = ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_HERO_KILLSOUND + "Victory", ResourceType.ASSET);
                AudioClip clip = clipUnit.Asset as AudioClip;

                Thanos.AudioManager.Instance.PlayLongVoiceAudio(clip);
            ...
            else if (player.StateSituation == SITUATION.LOSE)
            {
                ...
                //HolyTech.AudioManager.Instance.PlayLongVoiceAudio(Resources.Load(AudioDefine.PATH_HERO_KILLSOUND + "Failed") as AudioClip);

                ResourceItem clipUnit = ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_HERO_KILLSOUND + "Failed", ResourceType.ASSET);
                AudioClip clip = clipUnit.Asset as AudioClip;
