...
public class UIStartSelfDefine : MonoBehaviour {
    ...
    void OnEnable()
    {
        ...
        DoMoveIn();
        //UIGuide
        IGuideTaskManager.Instance().AddTaskStartListerner((Int32)GameEventEnum.GameEvent_UIGuideSelectPrimaryBtnStart, StartIGuideTask);
        IGuideTaskManager.Instance().AddTaskStartListerner((Int32)GameEventEnum.GameEvent_UIGuideCreateRoomBtnStart, StartIGuideTask);
        IGuideTaskManager.Instance().SendTaskEnd((Int32)GameEventEnum.GameEvent_UIGuideTriggerCreateRoom);
    }
    ...
    void StartIGuideTask(Int32 taskId)
    {
        ...
        {
            case GameEventEnum.GameEvent_UIGuideSelectCreateRoomBtnEnd:
                IGuideTaskManager.Instance().SendTaskEffectShow((Int32)GameEventEnum.GameEvent_UIGuideSelectCreateRoomBtnEnd);
                objList.Add(BtnCreateRoom.gameObject);
                break;
            case GameEventEnum.GameEvent_UIGuideSelectPrimaryBtnEnd:
    ...
    void MoveBackFinish() { 
		IGuideTaskManager.Instance().SendTaskEffectShow((Int32)GameEventEnum.GameEvent_UIGuideSelectPrimaryBtnEnd);
       // EnableBoxEvent(true);
    }
    ...
    void AddEvent() {
        ...
        EventCenter.AddListener<ErrorCodeEnum>((Int32)GameEventEnum.AskAddInBattle, StartSelfResult);
	...
	void OnDisable(){
		...
		EventCenter.RemoveListener<ErrorCodeEnum>((Int32)GameEventEnum.AskAddInBattle, StartSelfResult);
		IGuideTaskManager.Instance().RemoveTaskStartListerner((Int32)GameEventEnum.GameEvent_UIGuideSelectPrimaryBtnStart, StartIGuideTask);
        IGuideTaskManager.Instance().RemoveTaskStartListerner((Int32)GameEventEnum.GameEvent_UIGuideCreateRoomBtnStart, StartIGuideTask);
	}
	void BtnCreateOrAdd()
	{
	...
	void StartSelfResult(ErrorCodeEnum m_n32ErrorId)
	{
        switch (m_n32ErrorId)
        {
            case ErrorCodeEnum.AddBattleFailForAllFull://消
                ...
                break;
            case ErrorCodeEnum.AddBattleFailForUserFull://弹
...
//	}
	private void StartBtnOnPress(int ie, bool press)
	{
        ...
        {
            IGuideTaskManager.Instance().SendTaskEnd((Int32)GameEventEnum.GameEvent_UIGuideSelectPrimaryBtnEnd);
        }
	}
	public void CreateOrAddRoom(){
		if(string.Equals(BtnCreateRoom.GetComponentInChildren<UILabel>().text,CreateRoom))
		{
            IGuideTaskManager.Instance().SendTaskEnd((Int32)GameEventEnum.GameEvent_UIGuideSelectCreateRoomBtnEnd);
			if(string.IsNullOrEmpty(RoomPassWordLabel.text))
				HolyGameLogic.Instance.CreateRoom((int)TypeMap,"");
			else
				HolyGameLogic.Instance.CreateRoom((int)TypeMap,RoomPassWordLabel.text);
		}else{
            if (string.IsNullOrEmpty(InputRoomNum.label.text) || string.Equals(InputRoomNum.label.text, "-------"))
            {
                MsgInfoManager.Instance.ShowMsg((int)ErrorTypeEnum.NullRoomID);
            ...
            else
            {
                ...
                HolyGameLogic.Instance.AddRoom(RoomNumberLabel.text, RoomPassWordLabel.text);
            }
		}
