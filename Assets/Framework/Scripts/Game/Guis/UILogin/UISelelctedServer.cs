...
public class UISelelctedServer : MonoBehaviour
{
    ...
    //显示所有的服务器
    void ShowAllServer(int type = 0)
    { 
        // type = 0  show name,type = 1 show addr
    ...
    void CreateServerItems() {
        //create items ,and label 
        int count = SelectServerData.Instance.GetServerDicInfo().Count / 6;
        if (SelectServerData.Instance.GetServerDicInfo().Count % 6 != 0)
            count += 1;
        for (int i = 0; i < count * 6; i++) {
            ResourceItem sceneRootUnit = ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadServerItems, ResourceType.PREFAB);
            GameObject obj = Instantiate(sceneRootUnit.Asset) as GameObject;
    ...
    void ShowDefaultServer(int type = 0) {// type = 0  show name,type = 1 show addr  
        ...
        nameDefault.text = (type == 0) ? "自己的服务器"/*info.name */: info.addr;        
