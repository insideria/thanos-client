...
//		SetWin ();
	...
	void GameOverError(ErrorCodeEnum errorCode)
	{
	}
    ...
    public void Show()
    {
        ...
        {
            if ((EntityCampTypeEnum)item.HeroCampID == EntityCampTypeEnum.A)
            {
                if(i <= 2)
                    ListPlayer[i++].SetShowLabel(item.HeadIcon, item.HeadName, item.HeroLevel, item.HeroKills, item.HeroDeadTimes, item.HeroCampID);
                ShowBattleResult();
            }
            else if ((EntityCampTypeEnum)item.HeroCampID == EntityCampTypeEnum.B)
            {
                if(num <=5 )
...
//			HolyTechGameBase.Instance.SetGameState (GameDefine.GameState.STATE_LOBBY);
