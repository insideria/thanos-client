...
public class BuffUIItem
{
    ...
    public Thanos.Skill.Buff buff = null;
}
...
public class UIBuffUnityInterface : MonoBehaviour
{
    ...
    public void UpdateUIItem()
    {
        foreach (BuffUIItem bItem in buffUIItemList)
        {
            if (bItem.buff != null)
            {
                bItem.effect.fillAmount = (bItem.buff.GetTotalTime() - bItem.buff.Time) / bItem.buff.GetTotalTime();
            }
        }
    }
    //
    public void RefreshUIItem()
    {
        ...
        //return;
        int index = 0;
        foreach (Buff b in BuffManager.Instance.mBuffDict.Values)
        {
            if (index < 14)
            {
