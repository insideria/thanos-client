...
public class UIShop : MonoBehaviour {
    ...
    void SendOpenShop(bool tag) {
        Thanos.FEvent eve = new Thanos.FEvent((Int32)(Int32)GameEventEnum.GameEvent_NotifyOpenShop);
    ...
    void Awake() {
        labelCp = transform.Find("CPShow/CP/Label").GetComponent<UILabel>();//商店显示玩家的金币
    ...
    private bool CheckCanBuy() {
        ...
        {
            MsgInfoManager.Instance.ShowMsg((int)ErrorTypeEnum.NeedAItermID);
            return false;
        }
    ...
    {        
        HolyGameLogic.Instance.EmsgToss_AskBuyGoods(item);
    }
    ...
    public void OnUserSellItem()
    {
        ...
        if (isEmpty)
        {
            MsgInfoManager.Instance.ShowMsg((int)ErrorTypeEnum.BagIsEmpty);
        ...
        if (ShopItemBase.SelectIndex == null || ShopItemBase.SelectIndex.Count == 0)
        {
            MsgInfoManager.Instance.ShowMsg((int)ErrorTypeEnum.NeedABagID);
            return;
        }
