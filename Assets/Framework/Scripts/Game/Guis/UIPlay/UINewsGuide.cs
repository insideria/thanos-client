...
public class UINewsGuide : MonoBehaviour
{
    ...
    {
        Instance = this;
        //EventCenter.AddListener<UInt64, int, int, int>((Int32)GameEventEnum.GameEvent_GuideKillTask, OnEvent);
        EventCenter.AddListener<bool>((Int32)GameEventEnum.GameEvent_GuideShowBuildingTips, OnShowBuildingTips);
        isSet = false;
    }

    void OnDisable() {
        //EventCenter.RemoveListener<UInt64, int, int, int>((Int32)GameEventEnum.GameEvent_GuideKillTask, OnEvent);
        EventCenter.RemoveListener<bool>((Int32)GameEventEnum.GameEvent_GuideShowBuildingTips, OnShowBuildingTips);
    }
    ...
    void OnShowBuildingTips(bool show) {
        if (show)
        {
            ...
            for (int i = 0; i < dic.Count; i++)
            {
				...
				ResourceItem objUnit = ResourcesManager.Instance.loadImmediate(GuideBuildingTips.Instance.pathArray[index], ResourceType.PREFAB);
                GameObject obj = GameObject.Instantiate(objUnit.Asset) as GameObject;
                obj.transform.parent = BloodBarManager.Instance.transform; 
