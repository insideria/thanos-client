...
public class UIViewerPlayerPackage : MonoBehaviour {
    ...
    void OnEnable()
    {
        Instance = this;
        Init();
        EventCenter.AddListener<IPlayer>((Int32)GameEventEnum.GameEvent_PackageBuyInfo, SetPlayerage);
    }
    void OnDisable()
    {
        Instance = null;
        EventCenter.RemoveListener<IPlayer>((Int32)GameEventEnum.GameEvent_PackageBuyInfo, SetPlayerage);
    }
    ...
    {
        int seat = PackageDataManager.Instance.Seat;
        int id = PackageDataManager.Instance.Id; 
        int num = PackageDataManager.Instance.Num; 
        float totalTime = PackageDataManager.Instance.TotalTime;
        float lastTime = PackageDataManager.Instance.LastTime;
