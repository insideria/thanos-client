...
namespace Thanos.Ctrl
{
    public class RoomCtrl : Singleton<RoomCtrl>
    {
        public void Enter()
        {
            EventCenter.Broadcast((Int32)GameEventEnum.GameEvent_RoomEnter);
        }

        public void Exit()
        {
            EventCenter.Broadcast((Int32)GameEventEnum.GameEvent_RoomExit);
        }

        //改变座位
        public void ChangeSeat(int i)
        {
            HolyGameLogic.Instance.ChangeRoomSeat(i);
        }

        //开始战场
        public void BeginBattle()
        {
            HolyGameLogic.Instance.StartRoom();
        }

        //准备
        public void ReadyRoom()
        {
            HolyGameLogic.Instance.ReadyRoom();
        }

        //取消准备
        public void CancelReadyRoom()
        {
            HolyGameLogic.Instance.CancelRoom();
        }
        ...
        public void SendTalkMessage(string msg)
        {
            ...
            HolyGameLogic.Instance.EmsgToss_AskSendRoomTalk((UInt32)utf8Bytes.Length * sizeof(byte), utf8Bytes);
        }

        //接收聊天消息
        public void RecvTalkMessage(uint index, string msg)
        {
            EventCenter.Broadcast<uint, string>((Int32)GameEventEnum.GameEvent_RecvChatMsg, index, msg);
        }
        ...
        public void UpdateRoomBaseInfo(UInt64 roomID, UInt32 mapID)
        {
            ...
            if (SceneGuideTaskManager.Instance().IsNewsGuide((int)mapID) == SceneGuideTaskManager.SceneGuideType.NoGuide)
            {
                ...
                EventCenter.Broadcast((Int32)GameEventEnum.GameEvent_EndWaiting);

                EventCenter.SendEvent(new FEvent((Int32)(Int32)GameEventEnum.IntoRoom));
            }
        }

        public void ExitInviteList()
        {
            EventCenter.Broadcast((Int32)GameEventEnum.GameEvent_RoomInviteExit);
        }
        public void OpenInviteList()
        {
            if (FriendManager.Instance.RoomInvite.Count > 0)
                EventCenter.Broadcast((Int32)GameEventEnum.GameEvent_RoomInviteEnter);
        }

        public void SendInviteList(int roomID,UInt64 sGUID)
        {
            HolyGameLogic.Instance.EmsgToss_AskInviteFriendsToBattle(roomID, sGUID);
        }

        public void AskCanInviteFriends()
        {
            FriendManager.Instance.RoomInvite.Clear();
            HolyGameLogic.Instance.EmsgToss_AskCanInviteFriends();
        }
    }
}
