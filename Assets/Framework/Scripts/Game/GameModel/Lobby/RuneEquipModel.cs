...
namespace Thanos.Model
{
    class RuneEquipModel : Singleton<RuneEquipModel>
    {
        ...
        public void UpdateRuneSlotInfo(uint runeid, int page, int pos)
        {
            ...
            EventCenter.Broadcast<uint, int, int>((Int32)GameEventEnum.GameEvent_RuneQuipUpdate, runeid, page, pos);
        }

        public void RemoveRune(int page, int pos)
        {
            runePgae2SlotInfoMap.Remove(getRealPos(page, pos));
            EventCenter.Broadcast<int, int>((Int32)GameEventEnum.GameEvent_RuneQuipUnload, page, pos);
        }
