	...
	public static class GameMethod
	{
        ...
        public static Camera GetUiCamera
		{
			get{
                ...
                {
                    UICamera.currentCamera = HolyTechUI.Instance.transform.Find("Camera").GetComponent<Camera>();
                }
				return UICamera.currentCamera;
			}
		}

		private static Grayscale CameraGray;
		public static Grayscale GetCameraGray
        {
            get
            {
                if (CameraGray == null)
                { 
					CameraGray = GetMainCamera.GetComponent<Grayscale>();
                }
                return CameraGray;
            }
        }
...
//				MonoBehaviour.DestroyImmediate()
		...
		public static IEnumerable<IEntity> GetEnemyItemListByRadius(IEntity self, float radius){
			var items = from item in EntityManager.AllEntitys.Values
                ...
                where item.entityType != EntityTypeEnum.Monster
                where IsEnemyCamp(self , item)
                where item.FSM != null
                where item.FSM.State != Thanos.FSM.FsmStateEnum.DEAD
        ...
        public static List<IEntity> GetEnemyItemListByRadiusWithoutMonsterBuilding(IEntity self, float radius)
        {
            var items = from item in EntityManager.AllEntitys.Values
                ...
                where item.FSM.State != Thanos.FSM.FsmStateEnum.DEAD
                where item.entityType != EntityTypeEnum.Monster
                where item.entityType != EntityTypeEnum.Building
                let distance = Vector3.Distance(item.realObject.transform.position, self.realObject.transform.position)
                where distance < radius
                orderby distance
                select item;
        ...
        public static List<IEntity> GetSoldierEnemyItemListByRadius(IEntity self, float radius)
        {
            var items = from item in EntityManager.AllEntitys.Values  //创建查询 数据源:AllEntitys
                        ...
                        where item.FSM.State != Thanos.FSM.FsmStateEnum.DEAD//实体不是死亡状态
                        where item.entityType != EntityTypeEnum.Building//实体类型不是建筑
						where item.entityType != EntityTypeEnum.Monster//实体类型不是野怪
						where item.entityType != EntityTypeEnum.Player //实体类型不是玩家
                        let distance = Vector3.Distance(item.realObject.transform.position, self.realObject.transform.position)
        ...
        public static List<IEntity> GetMonsterEnemyItemListByRadius(IEntity self, float radius)
        {
            var items = from item in EntityManager.AllEntitys.Values
                        ...
                        where item.FSM.State != Thanos.FSM.FsmStateEnum.DEAD
                        where item.entityType != EntityTypeEnum.Building
                        where item.entityType != EntityTypeEnum.Soldier
                        where item.entityType != EntityTypeEnum.Player
                        let distance = Vector3.Distance(item.realObject.transform.position, self.realObject.transform.position)
                        where distance < radius
                        orderby distance
                        select item;
        ...
        {
            if (target.EntityCamp == EntityCampTypeEnum.Kind)
            {
                return false;
            }
            if (target.EntityCamp == EntityCampTypeEnum.Bad)
            {
                return true;
            }
		...
		public static GameObject CreateWindow(string name, Vector3 v3 , Transform father)
		{
            ...
            ResourceItem objUnit = ResourcesManager.Instance.loadImmediate(name, ResourceType.PREFAB);
        ...
        public static string GetCurrSystemTime(int hour,int minutes,int seconds) {
            ...
            if (hour != 0)
            {
                currTime += hour * TIMEBUCKET * TIMEBUCKET;//小时转换为秒
            }
            if(minutes != 0)
            {
                currTime += minutes * TIMEBUCKET;//分钟转换为秒
            }
            currTime += seconds;//得到时间的总秒数
            seconds = currTime % 60;//得到秒数
            minutes = currTime / 60;//得到分钟数
...
//#endif
        ...
        public static void FileRead()
        {
            ...
            //加载过滤配置文件
            ResourceItem xmlfileUnit = ResourcesManager.Instance.loadImmediate("Config/filter", ResourceType.ASSET);
        ...
        public static string GetSplitString(string currStr)
        {
            foreach (string item in split)
            {
                int currPos = currStr.IndexOf(item, StringComparison.OrdinalIgnoreCase);
